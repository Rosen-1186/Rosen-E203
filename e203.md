...
### 26/2/23
**当日事项**：
    1.基于 GTKWave 查看并分析仿真波形（仿真内容包括简单 ALU 运算与逻辑）
...
**踩坑**：在编译运行移位器的c代码时，为了查看波形搜索了机器码。手算的 32 位移位机器码是 00209093，然而 GTKWave 内检查时在 ifu_o_ir 检查时并未检出预期机器码。
**问题分析**：触发了 RISC-V 的 C 压缩指令集特性，把它活生生压成了一个 16 位指令：008a。
**解决方法**：使用 objdump 反编译 .elf 文件得到 .dump 文件后查找有关主函数机器码，分析其与32位标准机器码的不同。
**使用指令**：
```bash
